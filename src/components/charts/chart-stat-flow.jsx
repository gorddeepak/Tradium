"use client";;
import NumberFlow from "@number-flow/react";
import { useEffect, useMemo, useState } from "react";
import { cn } from "@/utils/utils";

export const defaultChartStatFlowFormat = {
  notation: "standard",
  maximumFractionDigits: 0,
};

/* Pinned rather than left to the browser. Every number in this app is rupees and
   is formatted en-IN elsewhere (see fmtMoney), and en-IN groups digits by lakh —
   1,23,456, not 123,456. Passing undefined here would make the grouping depend
   on the viewer's machine, so the same figure would render two different ways in
   the same card. */
const LOCALE = "en-IN";

function formatStatValue(value, formatOptions, prefix, suffix) {
  const formatted = new Intl.NumberFormat(LOCALE, formatOptions).format(value);
  return `${prefix ?? ""}${formatted}${suffix ?? ""}`;
}

function useNumberFlowElementReady() {
  const [ready, setReady] = useState(() =>
    typeof customElements !== "undefined" &&
    Boolean(customElements.get("number-flow-react")));

  useEffect(() => {
    if (ready) {
      return;
    }
    let cancelled = false;
    customElements.whenDefined("number-flow-react").then(() => {
      if (!cancelled) {
        setReady(true);
      }
    });
    return () => {
      cancelled = true;
    };
  }, [ready]);

  return ready;
}

/**
 * Shared value + label stack using NumberFlow (same layout as pie / ring centers).
 * Parent should provide flex alignment and sizing when needed.
 */
export function ChartStatFlow({
  value,
  label,
  formatOptions = defaultChartStatFlowFormat,
  prefix,
  suffix,
  valueClassName = "text-2xl font-bold",
  labelClassName = "text-xs",
  icon
}) {
  const numberFlowReady = useNumberFlowElementReady();
  const staticValue = useMemo(
    () => formatStatValue(value, formatOptions, prefix, suffix),
    [value, formatOptions, prefix, suffix]
  );

  return (
    <>
      {icon ? (
        <div
          className="mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-muted/50">
          {icon}
        </div>
      ) : null}
      <span className={cn("text-foreground tabular-nums", valueClassName)}>
        {numberFlowReady ? (
          <NumberFlow
            format={formatOptions}
            isolate
            locales={LOCALE}
            prefix={prefix}
            suffix={suffix}
            value={value}
            willChange />
        ) : (
          staticValue
        )}
      </span>
      <span className={cn("mt-0.5 text-chart-label", labelClassName)}>
        {label}
      </span>
    </>
  );
}

ChartStatFlow.displayName = "ChartStatFlow";
